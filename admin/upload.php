...
if (!empty($_FILES)) {
    ...
    $s_id = $_POST['s_id'];
    $folder_path = '../uploads/pro_g/' . $s_id;
    if (!file_exists($folder_path)) {

        mkdir($folder_path, 0777, true);
    }
    include_once('include/db.php');
    $targetDir = '../uploads/pro_g/' . $s_id . '/';
    ...
    if ($_FILES["file"]["error"] > 0) {
        ...
        $conn = $pdo->open();
        try {
			$stmt = $conn->prepare("INSERT INTO spro_images (s_id,file_name,uploaded_img_date) VALUES('" . $s_id . "','" . $fileName . "','" . $dt . "')");
            $stmt->execute();
