...
<body class="hold-transition skin-blue sidebar-mini">
                            ...
                            <div class="box-body">
                                <table id="example1" class="table table-bordered">
                                    ...
                                    <tbody>
                                        ...
                                        try {
                                            $now = date('Y-m-d');
                                            $stmt = $conn->prepare("SELECT * FROM pages_detail");
                                            $stmt->execute();
                                            foreach ($stmt as $show11) {
                                                ...
                                                if ($t_img != "") {
                                                    $cust_image = "../uploads/pages/" . $t_img;
                                                } else {
                                        ...
                                        <tr>
                                         <td>" . $t_name . "</td>
                                         <td><img src='" . $cust_image . "' height='50px'></td>";
                                                echo'<td>
                                        <form style="display: inline-block;"  method="post" action="edit_pages.php">
                                        <input type="hidden" name="edit_pages_id" value="' . $show11['id'] . '">
                                        <button type="submit" name="edit_pages_button" class="btn-success"><i class="fa fa-pencil"></i></button>
                                        </form>
                                        <form style="display: inline-block;"  method="post" action="add_inner_pages.php">
                                        <input type="hidden" name="add_inner_pages_id" value="' . $show11['id'] . '">
                                        <input type="hidden" name="add_inner_pages_" value="' . $show11['t_name'] . '">
                                        <button type="submit" name="add_pages_button" class="btn-success">Add Sub Tab</button>
                                        </form>
                                        <form style="display: none;margin-left: 10px;" action="" method="post">
    ...
    if (isset($_POST['home_details_submit'])) {
        ...
        $conn = $pdo->open();
        $ins_q = "INSERT into pages_detail(img,admin_id,t_name,t_about,designation,added_date) VALUES('" . $img_name . "'," . $admin_id . ",'" . $t_name . "','" . $t_about . "','" . $t_designation . "',now())";
    ...
    if (isset($_POST['delete_rate_button'])) {
        ...
        $select = $conn->prepare("SELECT id,img FROM pages_detail WHERE id='" . $delete_id . "'");
        $select->execute();
        $dell = $select->fetch();
        $del_file = $dell['img'];
        $pro_id = $dell['property_id'];
        $delete = $conn->prepare("DELETE FROM pages_detail WHERE id=$delete_id");
        $delete->execute();
        if ($delete) {
            $FileName = '../uploads/pages/' . $del_file;
