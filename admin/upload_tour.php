...
if (!empty($_FILES)) {
	...
	$s_id = $_POST['s_id'];
	$folder_path = '../uploads/tour/' . $s_id;
	if (!file_exists($folder_path)) {

		mkdir($folder_path, 0777, true);
	}
	include_once('include/db.php');
	$targetDir = '../uploads/tour/' . $s_id . '/';
	...
	($_FILES["file"]["type"] == "image/pjpeg")) {
		...
		$conn = $pdo->open();
		try {
			$stmt = $conn->prepare("INSERT INTO pro_images (s_id,file_name,uploaded_img_date) VALUES('" . $s_id . "','" . $fileName . "','" . $dt . "')");
			$stmt->execute();
