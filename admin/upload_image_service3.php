...
if ($f_id_data === '') {
    echo "<script>window.location = 'conscious_consumption.php'</script>";
}
?>
		...
		<div class="content-wrapper">
			...
			<section class="content-header">
				...
				<ol class="breadcrumb">
					...
					<li class="active"><?= $f_name ?> Images</li>
				</ol>
			</section>
					...
					<div class="col-md-12 board">
						...
						<div class="tab-pane" id="uidesign">
							...
							<div class="image_upload_div">
								<form action="upload3.php" method="post" class="dropzone" id="HomeSlideGallery">
							...
							<form name="r" id="form_mul_ref" method="post" action="">
								<div class="buttonContainer" style="text-align: center;">
									...
									<p>
										<a href="conscious_consumption.php" class="btn btn-success green">
											<i class="fa fa-arrow-circle-left"> Back to Gallery</i></a>
							...
							<form name="k" id="form_mul_del" method="post" action="">
								<ul class="img_ul" id="sortable">
									...
									if (!empty($f_id_data)) {
										...
										try {
											$now = date('Y-m-d');
											$stmt = $conn->prepare("SELECT * FROM gallery3 WHERE admin_id='" . $f_id_data . "' ORDER BY image_id ASC");
											$stmt->execute();
											foreach ($stmt as $row) {

											echo '<li class="img_li" id="menu-order-' . $row['image_id'] . '">
                                            <figure style="height: auto">
                                                <img src="../uploads/consious/' . $row['admin_id'] . '/' . $row['file_name'] . '" />
                                            </figure>
                                            <input type="hidden" class="dprog_id" value="' . $row['admin_id'] . '" />
                                            <div>
