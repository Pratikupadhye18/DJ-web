...
<body class="hold-transition skin-blue sidebar-mini">
    <div class="wrapper">
        ...
        <div class="content-wrapper">
          ...
          ";
                ...
                ?>
                <?php
                if (isset($_POST['edit_pages_button'])) {
                    $conn = $pdo->open();
                    $admin_id = 1;
                    $r_id = $_POST['edit_pages_id'];
                    $fetch = $conn->prepare("SELECT * FROM pages_detail WHERE id='" . $r_id . "'");
                    $fetch->execute();
                    foreach ($fetch as $show) {
                        ...
                        $image = (!empty($t_img)) ? '../uploads/pages/' . $t_img : '../images/user.jpg';
                    }
    ...
    if (isset($_POST['home_details_submit'])) {
        ...
        $conn = $pdo->open();
        $select = $conn->prepare("SELECT * FROM pages_detail WHERE id='" . $t_id . "'");
        $select->execute();
        $row = $select->fetch();
        ...
        if ($row > 0) {
            ...
            $query_upd = "UPDATE pages_detail SET img='" . $img_name . "' , t_name='" . $t_name . "', designation='" . $t_designation . "', t_about='" . $t_about . "' WHERE id='" . $t_id . "'";
            $insert = $conn->prepare($query_upd);
            $insert->execute();
            if ($insert) {
                move_uploaded_file($tmp, "../uploads/pages/$img_name");
